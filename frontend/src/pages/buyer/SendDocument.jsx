...
import { FileText, MapPin, Send, User, X, Calculator, Map } from 'lucide-react';
...
const SendDocument = () => {
  ...
  const handlePickupLocationSelect = (location) => {
    if (location) {
      ...
      // Auto-calculate price if delivery address is also set
  ...
  const handleDeliveryLocationSelect = (location) => {
    if (location) {
      ...
      // Auto-calculate price if pickup address is also set
  ...
  // Helper functions from distanceCalculator
  ...
  const confirmDelivery = async () => {
    ...
    try {
      ...
      alert('Document delivery request created successfully! Fee: ₱50');
      navigate('/buyer');
    } catch (error) {
